...
fn auth_profile_matches_current(current: &AuthDotJson, candidate: &AuthDotJson) -> bool {
    let current_mode = resolved_auth_mode(current);
    let candidate_mode = resolved_auth_mode(candidate);
    match (current_mode, candidate_mode) {
        (CoreAuthMode::ApiKey, CoreAuthMode::ApiKey) => {
        ...
        (CoreAuthMode::AgentIdentity, CoreAuthMode::AgentIdentity) => {
            current.agent_identity == candidate.agent_identity
        }
        _ if current_mode.has_chatgpt_account() && candidate_mode.has_chatgpt_account() => {
...
mod tests {
    ...
    fn chatgpt_auth(account_id: &str, email: &str) -> AuthDotJson {
        ...
        AuthDotJson {
            auth_mode: Some(CoreAuthMode::Chatgpt),
            openai_api_key: None,
            tokens: Some(tokens),
            last_refresh: None,
            agent_identity: None,
