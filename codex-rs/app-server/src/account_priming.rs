...
impl AccountPrimingShared {
    ...
    async fn prime_profile(&self, auth: &AuthDotJson) -> Result<RateLimitSnapshot, IoError> {
        ...
        let refreshed_auth = load_auth_dot_json(
            temp_home.path(),
            store_mode,
            AuthKeyringBackendKind::default(),
        )?
    ...
    async fn run_prime_turn(&self, thread: &codex_core::CodexThread) -> Result<(), IoError> {
        ...
        let StartIfIdleSubmission::Started { turn_id } = submission else {
            return Err(IoError::other("priming thread rejected its initial turn"));
        };
    ...
    async fn fetch_rate_limits_for_auth(
        ...
        let client = backend_client_for_auth(
...
fn auth_store_mode(auth: &AuthDotJson) -> AuthCredentialsStoreMode {
    if matches!(auth.auth_mode, Some(CoreAuthMode::ChatgptAuthTokens)) {
...
fn backend_client_for_auth(
    ...
    let mut client = BackendClient::new(chatgpt_base_url.to_string(), http_client_factory)
    ...
    client = client.with_auth_provider(Arc::new(BearerAuthProvider::new(
